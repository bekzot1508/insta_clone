...
class User(AbstractUser, BaseModel):
    ...
    email = models.EmailField(null=True, blank=True, unique=True)# blank forma uchun, null baza uchun
    phone_number = models.CharField(max_length=13, blank=True, null=True, unique=True)
    photo = models.ImageField(upload_to="user_photos/", null=True, blank=True,
                              validators=[FileExtensionValidator(allowed_extensions=['jpg', 'png', 'jpeg', 'heic', 'heif'])])  # pip/pipenv install pillow, imageField dan foydalnilgani uchun
    ...
    def check_username(self):
        if not self.username:
            temp_username = f"instagram-{uuid.uuid4().__str__().split('-')[-1]}" # instagram-23324fgs
